...
class TrackManager:
    ...
    def get_reference(self, x_car, y_car, horizon):
        ...
        v_limit = np.clip(v_limit, 1.0, 4.0)
...
class CasadiMPC:
    def __init__(self):
        ...
        for k in range(self.N):
            ...
            damping = 5.0 * delta_u_acc + 500.0 * delta_u_steer
...
class LMPCNode(Node):
    def __init__(self):
        ...
        # Support F1TENTH pose input (PoseStamped) and compute velocity from position/time

        self.declare_parameter('pose_topic', '/f1tenth/pose')
        self.prev_pose = None
        ...
        def _pose_cb(msg):
            ...
            # initialize / unwrap yaw consistently with odom_callback
            ...
            # compute velocity from change in position & time
            ...
            if self.prev_pose_time is None:
                ...
                if dt <= 1e-6:
                    ...
                    # determine sign by projecting displacement onto heading
                    heading_dx = math.cos(self.unwrapped_yaw)
                    heading_dy = math.sin(self.unwrapped_yaw)
                    proj = dx * heading_dx + dy * heading_dy
                    v = speed if proj >= 0 else -speed

            self.prev_pose = (px, py)
            self.prev_pose_time = t
            self.current_state = np.array([px, py, v, self.unwrapped_yaw])

        # subscribe to pose topic in addition to the Odometry subscription (keeps compatibility)
        self.create_subscription(PoseStamped, self.get_parameter('pose_topic').value, _pose_cb, 10)
        self.last_yaw = 0.0
    ...
    def finish_lap(self):
        ...
        if self.lap_count >= 10:
            self.get_logger().info("RACE COMPLETE! Stopping Car.")

...
MAX_ACCEL = 5.0 
SAFETY_MARGIN = 0.25
LAP0_SPEED = 3.0  # Speed for the learning lap
...
class TrackManager:
    def __init__(self, filename, width=4.0):
        ...
        if os.path.exists(filename):
            ...
            # NOTE: Inversion removed based on your previous success
...
class LMPCLearner:
    def __init__(self):
        ...
        self.tree = None
        self.Ae = np.zeros((4,4)); self.Be = np.zeros((4,2)); self.Ce = np.zeros((4,1))
...
class CasadiMPC:
    def __init__(self):
        ...
        # Soft Constraints (Slack Variables) to prevent "Solver Failed"
        self.Sl_right = self.opti.variable(self.N + 1)
        self.Sl_left = self.opti.variable(self.N + 1)
        ...
        for k in range(self.N):
            ...
            cost += 5.0*delta_u_acc + 10.0*delta_u_steer + 0.1*self.U[0, k]**2
            # Slack Penalty (High cost for hitting walls)
        ...
        for k in range(self.N):
            ...
            # Dynamics (Nominal + Learned Error)
...
class LMPCNode(Node):
    ...
    def _pose_cb(self, msg):
        # Calculate State from AutoDRIVE Point Data (Calculating Yaw manually)
        px = msg.x; py = msg.y
    ...
    def control_loop(self):
        ...
        # --- KEY LOGIC: LAP 0 = SIMPLE MPC, LAP 1+ = LMPC ---
        if self.lap_count == 0:
            # Use ZERO for error model -> Becomes Simple MPC
            Ae_dummy = np.zeros((4,4)); Be_dummy = np.zeros((4,2)); Ce_dummy = np.zeros((4,1))
            u_opt, pred = self.mpc.solve(x, [rx, ry, rpsi], w_right, w_left, LAP0_SPEED, 
                                         Ae_dummy, Be_dummy, Ce_dummy, self.prev_u)
        else:
            # Use LEARNED error model -> Becomes LMPC
        ...
        # Normalize outputs for AutoDRIVE [-1, 1]
        ...
        self.pub_viz_path.publish(m); self.publish_global_track(); self.publish_lap_info()
    ...
    def publish_lap_info(self):
        ...
        if self.current_state is not None:
             ...
             msg.text = "MODE: Simple MPC (Lap 0)" if self.lap_count == 0 else f"MODE: LMPC (Lap {self.lap_count})"

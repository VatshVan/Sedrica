...
# --- LAP 0 SETTINGS ---
LAP0_SPEED = 3.0  # Constant speed for the first "Teaching" lap
...
class LMPCNode(Node):
    ...
    def control_loop(self):
        ...
        if self.prev_state is not None and self.is_racing:
             ...
             # Update Learning ONLY after Lap 0
        ...
        # --- MPC LOGIC SWITCH ---
        if self.lap_count == 0:
            # LAP 0: SIMPLE MPC (Kinematic Only)
            # We pass ZEROS for Ae, Be, Ce so it relies only on Physics equations
            ...
            # LAP 1+: LMPC (Learned Dynamics)
            # We pass the learned error models
    ...
    def publish_global_track(self):
        ...
        for i in range(0, len(self.track.path), 5):
             ...
             b_msg.points.append(Point(x=x - np.sin(yaw)*wl, y=y + np.cos(yaw)*wl))
             b_msg.points.append(Point(x=x - np.sin(yaw)*wl, y=y + np.cos(yaw)*wl + 0.1))
             b_msg.points.append(Point(x=x + np.sin(yaw)*wr, y=y - np.cos(yaw)*wr))
             b_msg.points.append(Point(x=x + np.sin(yaw)*wr, y=y - np.cos(yaw)*wr + 0.1))

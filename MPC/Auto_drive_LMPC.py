...
# --- LAP 0 SETTINGS ---
LAP0_SPEED = 0.5  # Slow, safe speed for data collection (m/s)
PID_KP = 0.8      # Steering gain for Lap 0

class PIDController:
    """ Simple Path Follower for Lap 0 """
    def get_action(self, x, y, yaw, ref_x, ref_y):
        # Calculate angle to the target reference point
        target_dx = ref_x - x
        target_dy = ref_y - y
        ...
        # Unwrap error (handle -pi to pi transitions)
        while yaw_error > math.pi: yaw_error -= 2 * math.pi
        while yaw_error < -math.pi: yaw_error += 2 * math.pi
        
        # P-Control for steering
        steer = np.clip(PID_KP * yaw_error, -MAX_STEER_RAD, MAX_STEER_RAD)
        
        # Simple P-Control for speed (Constant throttle roughly)
        throttle = 0.2 # 20% throttle for safety
        
        return np.array([LAP0_SPEED, steer]) # Return [v_ref, steer] (MPC returns [accel, steer])
...
class TrackManager:
    def __init__(self, filename, width=4.0):
        ...
        if os.path.exists(filename):
            ...
            # --- CHECK THIS: DELETE IF TRACK IS BACKWARDS ---
            # data = data[::-1] 
            # -----------------------------------------------

            if data.shape[1] >= 4:
                self.path = data[:, :2]; self.widths = data[:, 2:4] 
            else:
                self.path = data[:, :2]; self.widths = np.ones((data.shape[0], 2)) * (width / 2.0)
        else:
...
class LMPCLearner:
    ...
    def add_data(self, X, U, X_next, X_nom):
        ...
        print(f"Data Added. Memory Size: {self.X.shape[0]}")
...
class CasadiMPC:
    def __init__(self):
        ...
        for k in range(self.N):
            ...
            cost += 5.0*delta_u[0] + 10.0*delta_u[1]
    ...
    def solve(self, x0, ref, w_right, w_left, v_tgt, Ae, Be, Ce, u_prev):
...
class LMPCNode(Node):
    def __init__(self):
        ...
        self.mpc = CasadiMPC()
        self.pid = PIDController() # New PID for Lap 0
    ...
    def _pose_cb(self, msg):
        ...
        if self.prev_pose is not None:
             ...
             if math.hypot(dx, dy) > 0.001: raw_yaw = math.atan2(dy, dx)
    ...
    def control_loop(self):
        ...
        # We record data in Lap 0 AND Lap 1+
        if self.prev_state is not None and self.is_racing:
             ...
             # Only Learn (Update Ae/Be) AFTER Lap 0 is done
             if self.lap_count > 0 and len(self.lap_data['X']) % 10 == 0: 
                 self.learner.update_error_model(x)

        # --- LAP START LOGIC ---
        if not self.is_racing and idx > 50: 
            self.is_racing = True; self.get_logger().info(f"Lap {self.lap_count} STARTED!")
        ...
        if self.lap_count == 0:
            # LAP 0: Use PID (Simple & Safe)
            # Use the 5th point ahead as a target for smooth steering
            target_idx = 5
            u_opt_raw = self.pid.get_action(x[0], x[1], x[3], rx[target_idx], ry[target_idx])
            
            # Map PID output [speed, steer] to control inputs [accel, steer]
            accel_cmd = (u_opt_raw[0] - x[2]) * 2.0 # Simple P-controller for speed
            u_opt = np.array([accel_cmd, u_opt_raw[1]])
            
            # For visualization, just create a dummy straight prediction line
            pred = np.zeros((2, N_HORIZON))
            
        else:
            # LAP 1+: Use LMPC (Learned & Aggressive)
            u_opt, pred = self.mpc.solve(x, [rx, ry, rpsi], w_right, w_left, v_max * self.aggressiveness, 
                                         self.learner.Ae, self.learner.Be, self.learner.Ce, self.prev_u)
    ...
    def finish_lap(self):
        # Add all data from the completed lap to the Learner Memory
        ...
        self.get_logger().info(f"Lap {self.lap_count} COMPLETE. Data stored.")
        self.lap_count += 1
        
        # Increase aggressiveness for next lap (LMPC will try to go faster)
        if self.lap_count > 0:
            self.aggressiveness = min(1.0, self.aggressiveness + 0.1)
            self.get_logger().info(f"Switched to LMPC. Aggressiveness: {self.aggressiveness}")
    ...
    def publish_lap_info(self):
        ...
        if self.current_state is not None:
             ...
             mode = "MODE: PID (Lap 0)" if self.lap_count == 0 else f"MODE: LMPC (Lap {self.lap_count})"
             msg.text = mode
